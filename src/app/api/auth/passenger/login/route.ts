...
export async function POST(req: NextRequest) {
  try {
    ...
    logger.logApiRequest("POST", "/api/auth/passenger/login", undefined, { email });

    const authService = serviceFactory.createAuthService();
    const result = await authService.loginPassenger({ email, password });

    logger.logAuthEvent("passenger_login", result.userId, { email });
    ...
    logger.logApiError("POST", "/api/auth/passenger/login", error as Error);
