...
const carTypes = ['SWIFT', 'ETIOS', 'ERTIGA', 'INNOVA', 'TRAVELLER']
const bookingStatuses = ['PENDING', 'ACCEPTED', 'COMPLETED', 'CANCELLED', 'REJECTED']
...
function getRandomDate(daysBack = 30, daysForward = 30) {
...
async function main() {
  ...
  try {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      const actualFare = status === 'COMPLETED' ? estimatedFare + Math.floor(Math.random() * 200) - 100 : null
      ...
      await prisma.bookings.create({
        data: {
          ...
          specialRequests: Math.random() > 0.7 ? getRandomItem(specialRequests) : null,
          status,
          passengerId: passenger.id,
          driverId: driver?.id,
          acceptedAt: status !== 'PENDING' ? new Date(baseDateTime.getTime() + 5 * 60000) : null,
          completedAt: status === 'COMPLETED' ? new Date(baseDateTime.getTime() + 45 * 60000) : null,
          cancelledAt: status === 'CANCELLED' ? new Date(baseDateTime.getTime() + 10 * 60000) : null,
          cancellationNote: status === 'CANCELLED' ? getRandomItem(cancellationNotes) : null
        }
      })
